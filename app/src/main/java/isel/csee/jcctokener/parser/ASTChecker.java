...
//            vector = NodeType.searchType(tempNode, vector);
//
...
//        structureVector.add(vector);
    ...
    public boolean visit(ThrowStatement node) { // Type 19
        ...
        int[] structureVector = new int[30];
        structureVector = NodeType.searchType(node, structureVector);
        return super.visit(node);
    }

    @Override
    public boolean visit(CatchClause node) { // Catch 부분을 가져올 수 있음 -> Type 21
...
//        System.out.println(NodeType.searchType(node));
    ...
    public boolean visit(SimpleName node) { // 보류
        ...
        int[] vector = new int[30];
        ASTNode tempNode = node;
//        System.out.println(ASTNode.nodeClassForType(node.getParent().getNodeType()).getSimpleName());

        if(!(node.getParent() instanceof PackageDeclaration)) {
            while(tempNode != null && !(tempNode instanceof TypeDeclaration) && !(tempNode instanceof PackageDeclaration)) {
                vector = NodeType.searchType(tempNode, vector);
